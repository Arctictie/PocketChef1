...
//User is signed in use an intent to move to another activity
    ...
    public void generateBaseMeals() {
        ...
        for (int i = 0; i < meals.length - 1; i++) {
            ...
            meal.put(KEY_MEAL_NAME, meals[0]);
            meal.put(KEY_DESCRIPTION, meals[i + 1]);

            db.collection("defaultMeals").document("defaultlist1").collection("DefaultMeals").document(meals[0])
                    .set(meal);
        }
